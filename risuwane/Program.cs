...
while (true)
{
    ...
    Console.Clear();
    TimeSpan signal  = TimeSpan.FromSeconds(random.Next(5000,10000));
    ...
    while (stopwatch.Elapsed < signal && !isTooFast)
    {
        if (Console.KeyAvailable && Console.ReadKey().Key == ConsoleKey.Spacebar)
        {
            ...
            else
            {
                ...
                while (stopwatch.Elapsed < reactionTime && isTooSlow)
                {
                    if (Console.KeyAvailable&& Console.ReadKey().Key== ConsoleKey.Spacebar)
                    {
                        isTooSlow = false ;
                        requiredReactionTime = stopwatch.Elapsed;
                    }
                    Console.Clear();
                    if (isTooSlow)
                    {
                    ...
                    else
                    {
                        ...
                        Console.WriteLine($"Reaction Time :"  +  $"{reactionTime.TotalMilliseconds} millieseconds" );
                    }
